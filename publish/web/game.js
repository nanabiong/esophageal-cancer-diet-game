...
const BGM_GENERATED_LOOP_SECONDS = 8;
...
const ACT3_AUTO_EXIT_DELAY = 950;
...
const RESULT_GALAXY_LOCATING_DEFAULT_CONFIG = {
  ...
  resultDietGalaxy: {
    ...
    orbitScene: {
      ...
      orbitPlanets: [
        { id: "planet_1", x: 140, y: 720, size: 230 },
        { id: "planet_2", x: 610, y: 655, size: 460 },
        { id: "planet_3", x: 930, y: 350, size: 230 },
        { id: "planet_4", x: 1015, y: 90, size: 140 }
      ],
      focusCamera: {
        x: 760,
        y: 430,
...
let resultAtlasDownButton = null;
let resultAtlasUpButton = null;
...
let bgmAudio = null;
let bgmAudioUnavailable = false;
let bgmAudioStarted = false;
let bgmUserPaused = false;
let bgmToggleButton = null;
let bgmAudioContext = null;
let bgmGeneratedGain = null;
let bgmGeneratedTimer = null;
let bgmGeneratedStarted = false;

document.addEventListener("DOMContentLoaded", async () => {
  try {
    initializeBackgroundBgm();
...
function initializeBackgroundBgm() {
  if (bgmAudio || bgmToggleButton) {
    return;
  }
  ...
  bgmAudio.addEventListener("error", () => {
    bgmAudioUnavailable = true;
    if (!bgmUserPaused && bgmAudioStarted) {
      startGeneratedBackgroundBgm();
    }
  });

  bgmToggleButton = document.createElement("button");
  bgmToggleButton.type = "button";
  bgmToggleButton.className = "bgm-toggle-button";
  bgmToggleButton.setAttribute("aria-label", "暂停背景音乐");
  bgmToggleButton.textContent = "BGM";
  ...
  window.addEventListener("pointerdown", unlockBackgroundBgm, { once: true, passive: true });
  window.addEventListener("keydown", unlockBackgroundBgm, { once: true });
  document.addEventListener("visibilitychange", handleBgmVisibilityChange);
}

function unlockBackgroundBgm() {
  if (bgmUserPaused) {
    return;
  }
...
function playBackgroundBgm() {
  bgmAudioStarted = true;
  bgmUserPaused = false;
  updateBgmToggleButton();

  if (!bgmAudioUnavailable && bgmAudio) {
    bgmAudio.volume = BGM_VOLUME;
...
function updateBgmToggleButton() {
  ...
  bgmToggleButton.classList.toggle("is-paused", bgmUserPaused);
  bgmToggleButton.setAttribute("aria-pressed", bgmUserPaused ? "false" : "true");
  bgmToggleButton.setAttribute("aria-label", bgmUserPaused ? "播放背景音乐" : "暂停背景音乐");
}

function handleBgmVisibilityChange() {
  if (!bgmAudioStarted || bgmUserPaused) {
    return;
  }
...
function startGeneratedBackgroundBgm() {
  if (bgmGeneratedStarted || bgmUserPaused) {
    return;
  }
  ...
  if (!bgmAudioContext) {
    ...
    bgmGeneratedGain = bgmAudioContext.createGain();
    bgmGeneratedGain.gain.value = BGM_VOLUME * 0.34;
    bgmGeneratedGain.connect(bgmAudioContext.destination);
  }
  ...
  bgmGeneratedStarted = true;
  scheduleGeneratedBgmLoop();
  bgmGeneratedTimer = window.setInterval(
    scheduleGeneratedBgmLoop,
    BGM_GENERATED_LOOP_SECONDS * 1000
  );
}

function stopGeneratedBackgroundBgm() {
  bgmGeneratedStarted = false;
  if (bgmGeneratedTimer) {
    window.clearInterval(bgmGeneratedTimer);
    bgmGeneratedTimer = null;
  }
}

function scheduleGeneratedBgmLoop() {
  if (!bgmAudioContext || !bgmGeneratedGain || bgmUserPaused) {
    return;
  }

  const startTime = bgmAudioContext.currentTime + 0.05;
  const notes = [
    [261.63, 0],
    [329.63, 1],
    [392.00, 2],
  ...
  notes.forEach(([frequency, offset]) => {
    playGeneratedBgmTone(frequency, startTime + offset, 0.82);
  });
}

function playGeneratedBgmTone(frequency, startTime, duration) {
  const oscillator = bgmAudioContext.createOscillator();
  const toneGain = bgmAudioContext.createGain();

  oscillator.type = "sine";
  oscillator.frequency.setValueAtTime(frequency, startTime);
  toneGain.gain.setValueAtTime(0.0001, startTime);
  toneGain.gain.exponentialRampToValueAtTime(0.18, startTime + 0.08);
  toneGain.gain.exponentialRampToValueAtTime(0.0001, startTime + duration);
  oscillator.connect(toneGain);
  toneGain.connect(bgmGeneratedGain);
...
function mergeResultGalaxyLocatingConfig(config) {
  return {
    ...
    resultDietGalaxy: {
      ...
      orbitScene: {
        ...RESULT_GALAXY_LOCATING_DEFAULT_CONFIG.resultDietGalaxy.orbitScene,
        ...(config.resultDietGalaxy?.orbitScene || {}),
        dockedPlanet: {
          ...RESULT_GALAXY_LOCATING_DEFAULT_CONFIG.resultDietGalaxy.orbitScene.dockedPlanet,
          ...(config.resultDietGalaxy?.orbitScene?.dockedPlanet || {})
        },
        orbitStage: {
          ...RESULT_GALAXY_LOCATING_DEFAULT_CONFIG.resultDietGalaxy.orbitScene.orbitStage,
          ...(config.resultDietGalaxy?.orbitScene?.orbitStage || {})
        },
        orbitPlanets: config.resultDietGalaxy?.orbitScene?.orbitPlanets ||
          RESULT_GALAXY_LOCATING_DEFAULT_CONFIG.resultDietGalaxy.orbitScene.orbitPlanets,
        focusCamera: {
...
function startAct3Intro() {
  ...
  clearAct3IntroAutoAdvance();
  showIntroBubble("act3_intro_bubble_1");
  scheduleAct3IntroAutoAdvance("act3_intro_bubble_1", 850);
}
...
function scheduleAct2IntroAutoAdvance(targetState, text = "") {
  clearAct2AutoAdvance();
  const typewriterDuration = Math.max(0, Array.from(text).length * 76);
  const holdDuration = targetState === ACT2_STATES.LUNCH_PREP_SELECT ? 1350 : 1200;

  act2AutoAdvanceTimer = window.setTimeout(() => {
    act2AutoAdvanceTimer = null;
    if (currentPhase !== PHASES.ACT2_LUNCH) {
      return;
    }

    if (targetState === ACT2_STATES.LUNCH_WINDOW_INTRO && currentState === ACT2_STATES.LUNCH_INTRO_BUBBLE) {
      enterAct2LunchWindowIntro();
      return;
    }

    if (targetState === ACT2_STATES.LUNCH_PREP_SELECT && currentState === ACT2_STATES.LUNCH_WINDOW_INTRO) {
      transitionAct2IntroToChoice();
    }
  }, typewriterDuration + holdDuration);
}

function clearAct2AutoAdvance() {
  if (act2AutoAdvanceTimer) {
    window.clearTimeout(act2AutoAdvanceTimer);
    act2AutoAdvanceTimer = null;
  }
}
...
function isAct2IntroState(stateId) {
  return stateId === ACT2_STATES.LUNCH_INTRO_BUBBLE ||
    stateId === ACT2_STATES.LUNCH_WINDOW_INTRO;
}
...
function createAct2ObjectElement(objectId, objectConfig) {
  ...
  if (isAct2IntroState(currentState)) {
    element.classList.add("act2-lunch-intro-enter");
  }
...
function advanceIntroStep() {
  ...
  if (introStep === 1) {
    ...
    scheduleAct3IntroAutoAdvance(null, 950);
    return;
  }
  ...
  if (introStep === 2) {
    ...
    scheduleAct3IntroAutoAdvance("act3_intro_bubble_2", 850);
    return;
  }
...
function scheduleAct3IntroAutoAdvance(bubbleId = null, holdDuration = 800) {
  clearAct3IntroAutoAdvance();
  const bubbleConfig = bubbleId ? act3Layouts.introBubbles?.[bubbleId] : null;
  const text = bubbleConfig?.text || "";
  const speed = bubbleConfig?.typewriterSpeed ?? TYPEWRITER_SPEED;
  const typewriterDuration = Math.max(0, Array.from(text).length * speed);

  act3IntroAutoTimer = window.setTimeout(() => {
    act3IntroAutoTimer = null;
    if (currentPhase !== PHASES.INTRO || !isIntroActive || isIntroAnimating) {
      return;
    }
...
function clearAct3IntroAutoAdvance() {
  if (act3IntroAutoTimer) {
    window.clearTimeout(act3IntroAutoTimer);
    act3IntroAutoTimer = null;
  }
}
...
function scheduleAct3AutoExit() {
  clearAct3AutoExit();
  act3AutoExitTimer = window.setTimeout(() => {
    act3AutoExitTimer = null;
    if (!isAct3ScrollExitEnabled || hasAct3ExitStarted || isAct3ExitAnimating) {
      return;
    }

    startAct3ExitSequence();
  }, ACT3_AUTO_EXIT_DELAY);
}

function clearAct3AutoExit() {
  if (act3AutoExitTimer) {
    window.clearTimeout(act3AutoExitTimer);
    act3AutoExitTimer = null;
  }
}
...
function renderResultDietGalaxyLayout(layer) {
  ...
  layer.style.setProperty("--result-docked-planet-x", `${config.orbitScene.dockedPlanet.x}px`);
  layer.style.setProperty("--result-docked-planet-y", `${config.orbitScene.dockedPlanet.y}px`);
  layer.style.setProperty("--result-docked-planet-size", `${config.orbitScene.dockedPlanet.size}px`);
  layer.style.setProperty("--result-dock-duration", `${config.orbitScene.dockDuration || 760}ms`);
...
function renderResultOutcomePanelHost() {
  ...
  ensureResultAtlasNavigationControls();
  updateResultAtlasNavigationVisibility();
}
...
function enterResultGalaxyState() {
  ...
  resultGalaxyLayer?.classList.remove("is-orbit-scene");
  resultGalaxyLayer?.querySelector(".result-main-planet")?.classList.remove("result-main-planet--docked");
...
function enterResultOrbitScene() {
  if (currentPhase === PHASES.RESULT_ORBIT) {
    return;
  }

  currentPhase = PHASES.RESULT_ORBIT;
  currentState = RESULT_STATES.ORBIT_SCENE;
  isResultOrbitPlanetSelected = false;
  resultRiskLayer?.classList.remove("is-visible");
  dockResultMainPlanet();

  const config = getResultOrbitSceneConfig();
  window.setTimeout(() => {
    renderResultOrbitStage();
  }, config.dockDuration || 760);
}

function getResultOrbitSceneConfig() {
  return RESULT_GALAXY_LOCATING_CONFIG.resultDietGalaxy.orbitScene || {};
}

function dockResultMainPlanet() {
  const layer = ensureResultGalaxyLayer();
  const planet = layer.querySelector(".result-main-planet");

  layer.classList.add("is-orbit-scene");
  planet?.classList.add("result-main-planet--docked");
}

function renderResultOrbitStage() {
  const config = getResultOrbitSceneConfig();
  const stageConfig = config.orbitStage || {};
  const largestPlanet = findLargestOrbitPlanet(config.orbitPlanets || []);
  ...
  resultOrbitLayer.style.setProperty("--result-orbit-x", `${stageConfig.x || 0}px`);
  resultOrbitLayer.style.setProperty("--result-orbit-y", `${stageConfig.y || 0}px`);
  resultOrbitLayer.style.setProperty("--result-orbit-width", `${stageConfig.width || 1200}px`);
  resultOrbitLayer.style.setProperty("--result-orbit-height", `${stageConfig.height || 900}px`);
  resultOrbitLayer.style.setProperty("--result-orbit-rotate", `${stageConfig.rotate || 0}deg`);

  resultOrbitLayer.appendChild(createResultOrbitPath(config.orbitPlanets || []));

  (config.orbitPlanets || []).forEach((planetConfig) => {
    const isLargest = planetConfig.id === largestPlanet?.id;
    const planet = document.createElement(isLargest ? "button" : "div");

    planet.className = `result-orbit-planet${isLargest ? " result-orbit-planet--largest" : ""}`;
    planet.dataset.planetId = planetConfig.id;
    planet.style.setProperty("--result-planet-x", `${planetConfig.x}px`);
    planet.style.setProperty("--result-planet-y", `${planetConfig.y}px`);
    planet.style.setProperty("--result-planet-size", `${planetConfig.size}px`);

    if (isLargest) {
      planet.type = "button";
      planet.addEventListener("click", handleLargestOrbitPlanetClick);
    }

    resultOrbitLayer.appendChild(planet);
  });

  ensureResultGalaxyLayer().appendChild(resultOrbitLayer);
  window.requestAnimationFrame(() => {
    resultOrbitLayer?.classList.add("is-visible");
  });
}

function createResultOrbitPath(planets = []) {
  const pathLayer = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  const path = document.createElementNS("http://www.w3.org/2000/svg", "path");

  pathLayer.classList.add("result-orbit-path");
  pathLayer.setAttribute("viewBox", "0 0 1250 950");
  pathLayer.setAttribute("preserveAspectRatio", "none");
  path.setAttribute("d", createResultOrbitPathData(planets));
...
function createResultOrbitPathData(planets = []) {
  ...
  const extendedPoints = getExtendedOrbitPoints(points);
  let pathData = `M ${extendedPoints[0].x} ${extendedPoints[0].y}`;

  for (let index = 0; index < extendedPoints.length - 1; index += 1) {
    const p0 = extendedPoints[Math.max(0, index - 1)];
    const p1 = extendedPoints[index];
    const p2 = extendedPoints[index + 1];
    const p3 = extendedPoints[Math.min(extendedPoints.length - 1, index + 2)];
    const control1 = {
      x: p1.x + (p2.x - p0.x) / 6,
      y: p1.y + (p2.y - p0.y) / 6
    };
    ...
    pathData += ` C ${control1.x} ${control1.y}, ${control2.x} ${control2.y}, ${p2.x} ${p2.y}`;
  }

  return pathData;
}

function getExtendedOrbitPoints(points) {
  const first = points[0];
  const second = points[1];
  const beforeFirst = {
    x: first.x + (first.x - second.x) * 0.55,
    y: first.y + (first.y - second.y) * 0.55
  };
  const last = points[points.length - 1];
  const previous = points[points.length - 2];
  const afterLast = {
    x: last.x + (last.x - previous.x) * 0.75,
    y: last.y + (last.y - previous.y) * 0.75
  };

  return [beforeFirst, ...points, afterLast];
}

function findLargestOrbitPlanet(planets = []) {
  return planets.reduce((largest, planet) => {
    if (!largest || Number(planet.size) > Number(largest.size)) {
      return planet;
    }
...
function clearResultOrbitNodeTimer() {
  if (!resultOrbitNodeTimer) {
    return;
  }

  window.clearTimeout(resultOrbitNodeTimer);
  resultOrbitNodeTimer = null;
}
...
function ensurePlanetAtlasLayer() {
  ...
  resultAtlasUpButton = createResultAtlasNavigationButton(
    "result-atlas-nav-up",
    "返回星球结果页",
    () => returnFromPlanetAtlas()
  );
  planetAtlasLayer.appendChild(resultAtlasUpButton);
  objectLayer.appendChild(planetAtlasLayer);
  return planetAtlasLayer;
}

function ensureResultAtlasNavigationControls() {
  if (!resultAtlasDownButton) {
    resultAtlasDownButton = createResultAtlasNavigationButton(
      "result-atlas-nav-down",
      "进入数据分析页",
      () => startResultAtlasClickTransition()
    );
    resultAtlasDownButton.dataset.guide = "点击查看你的星球图谱分析";
    objectLayer.appendChild(resultAtlasDownButton);
  }

  ensurePlanetAtlasLayer();
  return {
    down: resultAtlasDownButton,
    up: resultAtlasUpButton
  };
}

function createResultAtlasNavigationButton(className, label, onClick) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = `result-atlas-nav ${className}`;
...
function updateResultAtlasNavigationVisibility() {
  const isAtlasActive =
    currentPhase === PHASES.RESULT_PLANET_ATLAS ||
    document.body.classList.contains("planet-atlas-active");
  const isResultGalaxyVisible =
    currentPhase === PHASES.RESULT_GALAXY ||
    currentState === RESULT_STATES.DIET_GALAXY ||
    Boolean(resultGalaxyLayer?.classList.contains("result-enter")) ||
    Boolean(objectLayer?.querySelector(".result-outcome-panel-host"));

  resultAtlasDownButton?.classList.toggle(
    "is-visible",
    isResultGalaxyVisible && !isAtlasActive && !resultAtlasClickTransitioning
  );
  resultAtlasUpButton?.classList.toggle(
    "is-visible",
    isAtlasActive
  );
}
...
function createGalaxyCenterPlanet() {
  ...
  planet.style.setProperty("--result-center-planet-live-x", "0px");
  planet.style.setProperty("--result-center-planet-live-y", "0px");
  planet.style.setProperty("--result-center-planet-live-rotate", "0deg");
  planet.style.setProperty("--result-center-planet-live-scale", "1");
...
function createResultFoodOrbit() {
  ...
  if ((!objectLayer && !galaxyLocatingLayer) || config.enabled === false) {
    return;
  }

  const orbitParent = objectLayer || galaxyLocatingLayer;

  document.querySelectorAll(".result-food-orbit").forEach((element) => element.remove());
  ...
  for (let index = 0; index < renderedSlotCount; index += 1) {
    ...
    slot.dataset.angle = String(startAngle + arcStep * index);
    slot.style.setProperty("--result-food-orbit-delay", `${index * 60}ms`);
    slot.title = food?.label || food?.id || `slot ${index + 1}`;
    label.className = "result-food-orbit-label";
    label.textContent = food?.label || "未选择";
    ...
    if (food?.asset) {
      ...
      image.addEventListener("error", () => {
        slot.classList.remove("has-food-image");
        image.remove();
        label.classList.add("is-visible");
      }, { once: true });
      slot.classList.add("has-food-image");
    ...
    info.className = "result-food-orbit-info";
    info.textContent = food?.label || "未选择";
  ...
  positionResultFoodOrbitSlots(orbit, {
    centerX,
    centerY,
    radiusX,
    radiusY,
...
function positionResultFoodOrbitSlots(orbit, config, queueOffset = 0) {
  const slotCount = Math.max(1, Number(config.slotCount || orbit.children.length || 1));
  const pathSlotCount = Math.max(slotCount, Number(config.pathSlotCount || slotCount));
  const arcStep = Number(config.arcStep || 20);

  orbit.querySelectorAll(".result-food-orbit-slot").forEach((slot, index) => {
    const baseAngle = Number(slot.dataset.angle || 0);
    const startAngle = baseAngle - arcStep * index;
    const queueIndex = ((index + queueOffset) % pathSlotCount + pathSlotCount) % pathSlotCount;
    const angle = startAngle + arcStep * queueIndex;
    const angleRad = (angle * Math.PI) / 180;
    const x = config.centerX + Math.cos(angleRad) * config.radiusX;
    const y = config.centerY + Math.sin(angleRad) * config.radiusY;
...
function stopResultFoodOrbitAnimation() {
  if (resultFoodOrbitAnimationFrame) {
    window.cancelAnimationFrame(resultFoodOrbitAnimationFrame);
  }
...
function startResultPlanetAnimation(planetBody, metrics) {
  ...
  const renderFrame = (time) => {
    ...
    resultPlanetLastRenderTime = time;
    ...
    if (planetElement) {
      ...
      const liveRotate = Math.sin(elapsed / 2400) * 0.4 * liveMotionScale;
      const liveScale = 1 + Math.sin(elapsed / 2600) * 0.018 * liveMotionScale;

      planetElement.style.setProperty("--result-center-planet-live-x", `${liveX.toFixed(2)}px`);
      planetElement.style.setProperty("--result-center-planet-live-y", `${liveY.toFixed(2)}px`);
      planetElement.style.setProperty("--result-center-planet-live-rotate", `${liveRotate.toFixed(3)}deg`);
      planetElement.style.setProperty("--result-center-planet-live-scale", liveScale.toFixed(4));
    }
